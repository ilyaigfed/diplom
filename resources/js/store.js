...
export default {
    ...
    getters: {
        ...
        RANDOM_PROFILES: state => {
            return state.random_profiles;
        },
        NEWEST_TRACKS: state => {
            return state.newest_tracks;
        },
        TOP_TEN: state => {
    ...
    mutations: {
        ...
        SET_PROFILE_LIKED: (state, payload) => {
            state.profile_liked = payload;
        },
        ...
        SET_RANDOM_PROFILES: (state, payload) => {
            state.random_profiles = payload;
        },
        SET_NEWEST_TRACKS: (state, payload) => {
            state.newest_tracks = payload;
        },
        SET_TOP_TEN: (state, payload) => {
            state.top_ten = payload;
        },
        SET_DISCO: (state, payload) => {
            state.disco = payload;
        },
        SET_CLASSIC: (state, payload) => {
            state.classic = payload;
        },
        SET_HEAVY: (state, payload) => {
            state.heavy = payload;
        },
        ...
        SET_PLAYING_TRACK: (state, payload) => {
            state.playing_track = payload;
        }
    },
    actions: {
        ...
        FOLLOW: (context, payload) => {
            axios.post(`/api/user/${payload}/following`).then(res => {
                ...
                } else if(context.state.random_profiles) {
                    for(let i = 0; i < context.state.random_profiles.length; i++) {
                        if(context.state.random_profiles[i].link === payload) {
                            context.state.random_profiles[i].followers_count += 1;
                            context.state.random_profiles[i].is_following = true;
                        }
                    }
                }
            });
            ...
            axios.delete(`/api/user/${payload}/following`).then(res => {
                if(context.state.profile) {
                    ...
                    for(let i = 0; i < context.state.random_profiles.length; i++) {
                        if(context.state.random_profiles[i].link === payload) {
                            context.state.random_profiles[i].followers_count -= 1;
                            context.state.random_profiles[i].is_following = false;
                        }
                    }
                }
            });
        ...
        GET_PROFILE_LIKED: (context, payload) => {
            axios.get(`/api/user/${payload}/liked_track`).then(res => {
                context.commit('SET_PROFILE_LIKED', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        ...
        GET_RANDOM_PROFILES: (context) => {
            axios.get('/api/user/profile/random').then(res => {
                context.commit('SET_RANDOM_PROFILES', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        GET_NEWEST_TRACKS: (context) => {
            axios.get('/api/compilation/ten').then(res => {
                context.commit('SET_NEWEST_TRACKS', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        GET_TOP_TEN: (context) => {
            axios.get('/api/compilation/winners').then(res => {
                context.commit('SET_TOP_TEN', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        GET_DISCO: (context) => {
            axios.get('/api/compilation/party').then(res => {
                context.commit('SET_DISCO', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        GET_CLASSIC: (context) => {
            axios.get('/api/compilation/classic').then(res => {
                context.commit('SET_CLASSIC', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        GET_HEAVY: (context) => {
            axios.get('/api/compilation/heavy').then(res => {
                context.commit('SET_HEAVY', res.data.data);
            }).catch(error => {
                console.log(error.response);
            });
        },
        DO_SEARCH: (context, payload) => {
            axios.get('/api/search?q='+payload).then(res => {
                if(res.data.tracks.length === 0 && res.data.profiles.length === 0 && res.data.playlists.length === 0) {
                    context.commit('SET_SEARCH', null);
                } else {
        ...
        GET_NOTIFICATIONS: (context) => {
            axios.get('/api/user/me/following/notification').then(res => {
