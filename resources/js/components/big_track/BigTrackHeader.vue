<template>
    <v-flex style="max-height: 253px;">
        <v-layout class="pa-4" id="bt-bg">
            <v-flex xs2>
                <v-img
                        :src="track.image ? '/storage/'+track.image: '/siteimages/blurred-track-ava.jpg'"
                        aspect-ratio="1"
        ...
        <img :src="track.image ? '/storage/'+track.image: '/siteimages/blurred-track-ava.jpg'" id="sad" v-show="false">
    </v-flex>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            play() {
                this.$store.commit('SET_PLAYING_TRACK', { tracks: [this.track], current: 0 });
            }
        }
    }
</script>
